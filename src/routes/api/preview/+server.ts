...
export const GET: RequestHandler = async ({ url, cookies, setHeaders }) => {
  ...
  if (type === 'page' && slug.startsWith('/event/')) {
    const eventPage = await getSanityServerClient(true).fetch(pageSlugQuery, {
      slug
    });

		if (!eventPage || !eventPage.slug) {
			throw error(401, 'No event page found');
		}
    ...
    isPreviewing = true;
		...
		// Set the redirect slug and append the isPreview query
		// param, so that the app knows it's a Sanity preview.
		redirectSlug = `${eventPage.slug}?isPreview=true`;
  }

  if (type === 'page') {
    const slugPage = await getSanityServerClient(true).fetch(pageSlugQuery, {
      slug
    });

		if (!slugPage || !slugPage.slug) {
			throw error(401, 'No slug page found');
		}
    ...
    isPreviewing = true;
		...
		// Set the redirect slug and append the isPreview query
		// param, so that the app knows it's a Sanity preview.
		redirectSlug = `${slugPage.slug}?isPreview=true`;
  }
