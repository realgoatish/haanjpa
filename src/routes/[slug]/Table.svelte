	...
	const sortRowsByIndex = (rows, sortedIndex, sortedDirection) =>
		rows.slice(0).sort((a, b) => {
			if (sortedDirection === 'ascending') {
				return a[sortedIndex] > b[sortedIndex] ? 1 : a[sortedIndex] < b[sortedIndex] ? -1 : 0;
			} else {
				return a[sortedIndex] < b[sortedIndex] ? 1 : a[sortedIndex] > b[sortedIndex] ? -1 : 0;
			}
		});
	...
	<table>
		...
		<tr>
			{#each tableData.headers as header, i}
				<th role="columnheader" scope="col" key={i} aria-sort={sortedBy === i ? sortDir : null}>
					{header}
					{#if sortable}
						<button on:click={() => sortBy(i)}>
							<SortArrow {sortDir} isCurrent={sortedBy === i} />
